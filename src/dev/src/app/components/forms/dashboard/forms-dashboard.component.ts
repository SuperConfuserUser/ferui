...
@Component({
  template: `
    ...
    <p>
      We've created a set of directives to help manage forms with minimal effort. The structure is more condensed and easier to
      implement, so it is the recommended approach to use the following if you are using Angular.
    </p>

    <h5>Basic forms</h5>
    <p>
      Declaring a form start by adding the <code>fuiForm</code> directive to the form element. This will wire up some internals to
      manage the form itself.
    </p>
    <pre><code [highlight]="basicFormExample"></code></pre>

    <h5>Basic structure</h5>
    <p>
      When you start to fill in your form controls, each will should be wrapped in a container like you see here in this text
      input example. If you don't, then you'll miss all the validation icons and messages. But your element will still work.
    </p>
    ...
    <p>
      ...
      validator name provided in the binding for <code>*fuiIfError="'errorName'"</code>, which might be your custom validator or a
      built in Angular one.
    </p>
    <pre><code [highlight]="multipleErrorMsgExample"></code></pre>

    <h5>Default error messages</h5>
    <p>
      Even if we recommend adding an error message for each validator you specify, we already have some defaults ones that you can
      rely on. We support all
      <a href="https://angular.io/api/forms/Validators" target="_blank">angular built-in validators</a> and some other custom ones
      that we've made (Ip-address validator...etc)
    </p>
    <pre><code [highlight]="defaultErrorMessages"></code></pre>

    <p class="alert alert-primary">
      Note that if you specify an error message without <code>*fuiIfError</code> value, it will override any other default ones,
      but if you specify a message for a specific validator name, then only this default validator message will be override.
    </p>
  `
})
